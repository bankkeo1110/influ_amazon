...
function parseInnertubeResults(data: { contents?: { twoColumnSearchResultsRenderer?: { primaryContents?: { sectionListRenderer?: { contents?: InnertubeItem[] } } } } }) {
  const items: {
    videoId: string;
    title: string;
    description: string;
...
async function searchYoutubeInnerTube(query: string) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const productName of products) {
      ...
      const videos = await searchYoutubeInnerTube(trimmed);
